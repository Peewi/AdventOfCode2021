...
Console.WriteLine("Advent of Code day 13 part 2");
...
{
	visualWidth = Math.Max(visualWidth, dots[i].X);
	visualHeight = Math.Max(visualHeight, dots[i].Y);
}
string[] visualLines = new string[visualHeight + 1];
...
for (int i = 0; i < visualLines.Length; i++)
{
	Console.WriteLine(visualLines[i]);
}